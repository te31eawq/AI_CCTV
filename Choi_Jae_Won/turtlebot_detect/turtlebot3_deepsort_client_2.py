    ...
    while True:
        ...
        for obj in tracked_objects:
            ...
            if id in tracked_vehicles:
                ...
                tracked_ids.add(id)  # 현재 추적 중인 차량 ID 추가
            else:
                tracked_vehicles[id] = {'centroid': (cx, cy), 'speed': 0}

        # 사라진 차량에 대한 처리: 추적되지 않는 차량에 대해서는 red_box_flags에서 삭제
        for id in list(red_box_flags.keys()):
            if id not in tracked_ids:
                del red_box_flags[id]  # 사라진 차량은 red_box_flags에서 제거

        # 사고가 감지되면 서버에 메시지 전송 (red_box_flags[id]가 True일 경우)
        if any(red_box_flags.values()) and not accident_sent:  # 빨간색 상자가 있는 차량이 있으면 사고 발생
            message_to_send = "Accident Detected\n"
            send_thread = threading.Thread(target=send_msg, args=(sock, message_to_send))
            send_thread.start()
        ...
        if not any(red_box_flags.values()) and not okay_sent:
